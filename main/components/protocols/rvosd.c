...
uint8_t res = 0;
  ...
  if (c == '$' && !frame_started){
    ...
    if (checksum_read == checksum_calculation){
      to_host_data.GPS_lat = ((int32_t)rvosd_lat_bp*100000+(int32_t)(rvosd_lat_ap/10))*(int32_t)latsign;
      to_host_data.GPS_lon = ((int32_t)rvosd_lon_bp*100000+(int32_t)(rvosd_lon_ap/10))*(int32_t)lonsign;
      telem_data.GPS_sats = satstemp;
      telem_data.Alt = altsign*alttemp;
      to_host_data.GPS_course = rvosd_headingtemp;
      // data is ready
      ///gotFix = true;
      ///gotAlt = true;
	  res = 1;
