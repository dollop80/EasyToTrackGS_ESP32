...
uint8_t processFrskyPacket(uint8_t *packet);
uint8_t parseTelemHubByte(uint8_t c);
...
uint8_t processHubPacket(uint8_t id, uint16_t value)
{
	uint8_t res = 0;
  if (id > FRSKY_LAST_ID)
    return res;
  ...
  if ((NS == 'N' || NS == 'S') && (EW == 'E' || EW == 'W')) {
	to_host_data.GPS_lat = frskyx_setLat();
	to_host_data.GPS_lon = frskyx_setLon();
	telem_data.GPS_sats = sats;
...
uint8_t processSportPacket(uint8_t *packet)
{
	uint8_t res = 0;
  ...
  if (!checkSportPacket(packet)){
	  if(telem_data.Errors < 100) telem_data.Errors++;
	  return res;
  }
      ...
      if ((appId >> 8) == 0) {
		...
		*/
        ...
        if ((NS == 'N' || NS == 'S') && (EW == 'E' || EW == 'W')) {
          ...
          telem_data.GPS_sats = sats;
		  res = 1;
		  if(telem_data.Errors > 0) telem_data.Errors--;
          ///gotFix = true;
        }
		  ...
		  telem_data.Alt = (int16_t) (msl_dc * pow(10,msl_x) * 0.1 * (msl_sgn == 1?-1:1));
    	  ///gotAlt = true;
		  res = 1;
		  if(telem_data.Errors > 0) telem_data.Errors--;
      }
...
uint8_t frskyx_parse(uint8_t data) {
  uint8_t res = 0;
  ...
  if (numPktBytes == FRSKY_RX_PACKET_SIZE) {
    res = processSportPacket(frskyx_RxBuffer);
