...
uint8_t frskyd_parse(uint8_t c) {
  uint8_t res = 0;
  switch (dataState) {
  ...
  case STATE_DATA_IN_FRAME:
    ...
    {
      res = processFrskyPacket(frskyRxBuffer); // FrskyRxBufferReady = 1;
		...
		case USRPKT:
			...
			for (uint8_t i = 3; i < numBytes+3; i++) {
				/*if(packet[3]=='$')
					...
					res = parseTelemHubByte(packet[i]);
			}
		break;
	}
	return res;
